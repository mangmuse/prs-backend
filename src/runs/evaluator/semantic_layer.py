...
def check_semantic(
    ...
    logger.debug(
        "Semantic 검증 시작 | schema=%s, threshold=%.2f", output_schema.value, threshold
    )
    ...
    try:
        ...
        logger.debug(
            "Embedding 완료 | raw_dim=%d, expected_dim=%d",
            len(raw_embedding),
            len(expected_embedding),
        )
    ...
    logger.debug(
        "Semantic 결과 | score=%.4f, threshold=%.2f, passed=%s",
        score,
        threshold,
        passed,
    )
