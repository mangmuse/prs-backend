...
class TestGetLLMClient:
    ...
    def test_gemini_provider_returns_gemini_client(self):
        ...
        with patch.dict(
            ...
            "src.llm.factory._resolve_api_key", return_value="resolved-key"
        ):
            from src.llm.factory import get_llm_client
...
class TestGetLLMClientWithApiKey:
    ...
    def test_api_key_is_passed_to_resolve(self):
        """외부 api_key가 _resolve_api_key에 전달되어야 한다."""
        mock_client_class = MagicMock()
        with patch.dict(
            "src.llm.factory.PROVIDER_CLIENTS",
            {"openai": mock_client_class},
        ), patch(
            "src.llm.factory._resolve_api_key", return_value="sk-test-key"
        ) as mock_resolve:
            from src.llm.factory import get_llm_client
    ...
    def test_api_key_none_calls_resolve_with_none(self):
        """api_key 미지정 시 _resolve_api_key에 None이 전달되어야 한다."""
        mock_client_class = MagicMock()
        with patch.dict(
            "src.llm.factory.PROVIDER_CLIENTS",
            {"openai": mock_client_class},
        ), patch(
            "src.llm.factory._resolve_api_key", return_value="env-key"
        ) as mock_resolve:
            from src.llm.factory import get_llm_client
...
class TestResolveApiKey:
    ...
    def test_gemini_provider_key_mapping(self):
        ...
        with patch("src.llm.factory.get_settings") as mock:
            ...
            result = _resolve_api_key("gemini", None)
